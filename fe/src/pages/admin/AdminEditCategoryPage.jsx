...
export default function AdminEditCategoryPage() {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            try {
                const data = await vehicleCategoryService.getById(id);
                const cat = vehicleCategoryService.normalizeCategory(data);
                setFormData({
                    brand: cat.brand,
                    name: cat.name,
                    model: cat.model,
                    seats: cat.seats || '',
                    batteryCapacityKwh: cat.batteryCapacityKwh || '',
                    rangeKm: cat.rangeKm || '',
                    chargingTimeHours: cat.chargingTimeHours || '',
                    description: cat.description || '',
                    imageUrls: cat.imageUrls.length ? cat.imageUrls : [''],
                    dailyPrice: cat.dailyPrice || '',
                    weeklyPrice: cat.weeklyPrice || '',
                    monthlyPrice: cat.monthlyPrice || '',
                    overtimeFeePerHour: cat.overtimeFeePerHour || '',
                });
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await vehicleCategoryService.update(id, payload);
    ...
    return (
        <div className="max-w-3xl mx-auto">
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Edit Vehicle Category</h1>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input name="brand" value={formData.brand} onChange={handleChange} required className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#5fcf86] focus:border-transparent" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Name (Line) *</label>
                        <input name="name" value={formData.name} onChange={handleChange} required className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#5fcf86] focus:border-transparent" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Model (Variant) *</label>
                        <input name="model" value={formData.model} onChange={handleChange} required className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#5fcf86] focus:border-transparent" />
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" disabled={saving || uploadingIndex !== null} className="px-6 py-2.5 text-sm font-semibold text-white bg-[#5fcf86] hover:bg-[#4ab872] rounded-lg transition-colors disabled:opacity-50">
